#ifndef PVZ2NATIVE_GFX_GLES_COMPAT_H
#define PVZ2NATIVE_GFX_GLES_COMPAT_H

#include <glad/gl.h>

/* No ancmp: we don't jump into this code directly from raw ARM machine code
 * (dynarmic marshals arguments from emulated registers itself), so no ARM
 * AAPCS/x86 calling-convention fixup is needed here. */
...
extern void gl_bind_attrib_location(GLuint program, GLuint index, const GLchar *name);
...
extern void gl_client_active_texture(GLenum texture);

extern void gl_compressed_tex_image_2_d(GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLint border, GLsizei imageSize, const GLvoid *data);

extern void gl_delete_framebuffers(GLsizei n, const GLuint *framebuffers);

extern void gl_disable_vertex_attrib_array(GLuint index);

extern void gl_framebuffer_texture_2_d(GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level);

extern void gl_front_face(GLenum mode);

extern void gl_gen_framebuffers(GLsizei n, GLuint *framebuffers);

extern void gl_get_integer_v(GLenum pname, GLint *params);

extern GLboolean gl_is_program(GLuint program);

extern GLboolean gl_is_shader(GLuint shader);

extern GLboolean gl_is_texture(GLuint texture);

extern void gl_load_matrix_f(const GLfloat *m) FLOAT_ABI_FIX;

extern void gl_pixel_storei(GLenum pname, GLint param);

extern void gl_tex_env_f(GLenum target, GLenum pname, GLfloat param) FLOAT_ABI_FIX;
