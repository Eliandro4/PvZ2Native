#include <pvz2native/audio/sound_repository.h>
...
static pvz2native_sound_t pvz2native_sound_repository[] = {
    {
        .name = "random.pop2",
        .resources_count = 1,
        .resources = {
...
pvz2native_sound_resource_t *pvz2native_get_sound_buffer(char *name) {
    for (int i = 0; pvz2native_sound_repository[i].name != NULL; ++i) {
        pvz2native_sound_t *sound = &pvz2native_sound_repository[i];
        if (strcmp(sound->name, name) == 0) {
            srand(time(NULL));
            int resource_index = rand() % sound->resources_count;
            pvz2native_sound_resource_t *resource = &sound->resources[resource_index];
            if (resource->buffer == NULL) {
                ...
                if (samples_count != -1) {
                    ...
                    memset(resource, 0, sizeof(pvz2native_sound_resource_t));
                }
            }
            return resource;
        }
