...
void gl_bind_attrib_location(GLuint program, GLuint index, const GLchar *name) {
...
void gl_client_active_texture(GLenum texture) {
    glClientActiveTexture(texture);
}

void gl_compressed_tex_image_2_d(GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLint border, GLsizei imageSize, const GLvoid *data) {
...
void gl_get_integer_v(GLenum pname, GLint *params) {
...
FLOAT_ABI_FIX void gl_load_matrix_f(const GLfloat *m) {
    glLoadMatrixf(m);
}
...
FLOAT_ABI_FIX void gl_tex_env_f(GLenum target, GLenum pname, GLfloat param) {
